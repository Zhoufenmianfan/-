...
for i = 1:length(L)
  ...
  [MSE2(i,j)] = channel_est_MIMO_LS(Ns,Nd,snr,L(i),lambda);
...
semilogy(L,mean(MSE2,2),'-');
semilogy(L,mean(MSE2_1,2),'-');
semilogy(L,mean(MSE2_2,2),'-');
semilogy(L,mean(MSE2_3,2),'-');
legend("LMS:u=0.05","LMS:u=0.1","LMS:u=0.2","LS:lambda=1","LS:lambda=2","LS:lambda=0.5","LS:lambda=0.2");
